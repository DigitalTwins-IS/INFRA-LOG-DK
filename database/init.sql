-- Unified SQL schema + seed data (PostGIS)
-- Generated by ChatGPT (merged from two provided files)
...
-- Tabla intermedia: inventario de cada tendero
CREATE TABLE IF NOT EXISTS shopkeeper_inventory (
    id SERIAL PRIMARY KEY,
    shopkeeper_id INTEGER NOT NULL REFERENCES shopkeepers(id) ON DELETE CASCADE,
...
CREATE TRIGGER update_shopkeeper_inventory_updated_at BEFORE UPDATE ON shopkeeper_inventory
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE INDEX IF NOT EXISTS idx_inventory_shopkeeper ON shopkeeper_inventory(shopkeeper_id);
CREATE INDEX IF NOT EXISTS idx_inventory_product ON shopkeeper_inventory(product_id);
...
SELECT 
    ...
    si.max_stock,
    CASE 
        WHEN si.stock < si.min_stock THEN 'low'
        WHEN si.stock > si.max_stock THEN 'high'
        ELSE 'normal'
    END as stock_status,
    si.last_updated,
    si.created_at
FROM shopkeeper_inventory si
JOIN shopkeepers sk ON si.shopkeeper_id = sk.id
JOIN products p ON si.product_id = p.id
WHERE sk.is_active = TRUE AND p.is_active = TRUE;
...
-- Shopkeepers (seed coordinates included). Use ON CONFLICT DO NOTHING
...
-- Inventory seed (from first file)
INSERT INTO shopkeeper_inventory (shopkeeper_id, product_id, stock, min_stock, max_stock) VALUES
    (1, 1, 50, 20, 100),
    (1, 2, 30, 10, 80),
    (1, 3, 25, 15, 60),
    (2, 3, 40, 20, 100),
